...
const shortlist_filename = "data/shortlist2" + cfg.xpid + ".json";
const prices_filename = "data/token_price" + cfg.xpid + ".json";
const routes_filename = "data/routes2" + cfg.xpid + ".json";
const last_run_filename = "data/last_run2" + cfg.xpid + ".txt";
...
let goodRoutes = JSON.parse(fs.readFileSync(infile));
...
async function simulateTrade(tri, input_dollars = "1") {
  ...
  const [dexa, dexb] = [tri.dexa, tri.dexb];
  const [token0, token1] = [tri.token0, tri.token1];
  ...
  try {
    ...
    let routeb = getRoute(dexb, addy1, addy0);
    ...
    try {
      ...
      n1_tokens = ethers.utils.formatUnits(n1_wei, token1_decimal);
    ...
    try {
      ...
      let [amount_in_token1, amount_out_token0] = amount_out_b;
      output_wei = amount_out_token0;
      //console.log("Wei out:", output_wei);
      output_tokens = output_wei * Math.pow(10, -token0_decimal);
      if (verbose) {
        console.log("Final sale", output_tokens, token0, dexb);
      }
    } catch (err) {
      //console.log(err);
      console.log("Final sale error", token1, token0, dexb);
    }
    ...
    let wei_outputs = { input_wei, n1_wei, output_wei };
    let trade_outputs = { input_tokens, n1_tokens, output_tokens };
  } catch (err) {
...
async function runSim(
  inputTriangles,
  outputCSV = "data/simulation2.csv",
  input_dollars = "10"
) {
  let resultsArray = [];
  let naArray = [];
...
async function main() {
  ...
  let csv_file_name = "data/simulation_2_" + currentTime + ".csv";
  let resultsArray = await runSim(goodRoutes, csv_file_name, "5");
  fs.writeFileSync(
    simulate_filename,
